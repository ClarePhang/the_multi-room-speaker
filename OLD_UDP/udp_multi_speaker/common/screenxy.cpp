...
#ifdef GL_DISPLAY
void screenxy::add(__useconds_t s, int diff)
{
    ...
    if(_m.try_lock())
    {
        ...
        glPushMatrix();
            ...
            for(int line=-20;line<20;line++){
                glVertex2f(0,line*(FRM_SCALE*2));
                glVertex2f(GRAPH_SAMPLES,line*(FRM_SCALE*2));
            }
            glEnd();
            char xout[32];
            int ql=0;
            for(int line=-20;line<20;line++){
                glRasterPos2f(0, line*(FRM_SCALE*2));
                sprintf(xout,"%d",ql++);
