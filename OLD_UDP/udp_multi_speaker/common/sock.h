/**
# Copyright (C) 2007-2015 s(Octavian Chincisan Marius) -  - N/A
#
...
public:
    ...
    static void Init();
    static void Uninit();
    static char*  GetLocalIP(const std::string reject);
    ...
    virtual int     sendall(const uint8_t* buff,  int length, int tout=8000)=0;
...
public:
    ...
    template<typename T>
    int             receiveall(T& buff){
        return receiveall((const uint8_t*)&buff,sizeof(buff));
    }
    template<typename T>
    int             send(const T& buff){
        return sendall((const char*)&buff,sizeof(buff));
    }
    char*           ssock_addrip();
...
public:
    ...
    virtual SOCKET  create(int port, int opt=0, const char* inetaddr=0);
    virtual SOCKET  create(const SADDR_46& r, int opt=0);
    virtual int     raw_connect(const SADDR_46& uip4, int tout=2);
...
public:
    ...
    virtual SOCKET  create(int port, int opt, const char* inetaddr=0);
    virtual SOCKET  create(const SADDR_46& r, int opt=0);
    virtual int     send(const char* buff, const int length, int port=0, const char* ip=0  );
...
class udp_group_sock : public udp_sock
{
    ...
    bool            _groupmember=false;

public:
    udp_group_sock(int opt=0){_option=opt;};
    virtual ~udp_group_sock(){destroy();};
    SOCKET     gcreate(const char* ipGrp, int port,const char* inetaddr=0);
    void    set_mtu(int mtu);
    int     join(const char* ipGrp, int port);
    void    drop();
    bool    is_set(int to);
    virtual bool    destroy(bool be=true);
    virtual int     send(const uint8_t* buff, int length, int port=0, const char* ip=0  );
    virtual int     receive(unsigned char* buff, int length, int port=0, const char* ip=0  );
};
